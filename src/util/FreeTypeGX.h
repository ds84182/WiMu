...
const GXColor ftgxBlack = (GXColor){0x00, 0x00, 0x00, 0xff}; /**< Constant color value used only to sanitize Doxygen documentation. */
...
class FreeTypeGX {
	...
	public:
		FreeTypeGX(const uint8_t* fontBuffer, FT_Long bufferSize, FT_UInt pixelSize, uint8_t vertexIndex = GX_VTXFMT1);
		~FreeTypeGX();

		void setVertexFormat(uint8_t vertexIndex);
		void setCompatibilityMode(uint32_t compatibilityMode);

		uint16_t drawText(int16_t x, int16_t y, wchar_t *text, GXColor color = ftgxBlack, uint16_t textStyling = FTGX_NULL);
		uint16_t drawText(int16_t x, int16_t y, wchar_t const *text, GXColor color = ftgxBlack, uint16_t textStyling = FTGX_NULL);
