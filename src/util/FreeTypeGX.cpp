 ...
 */
FreeTypeGX::FreeTypeGX(const uint8_t* fontBuffer, FT_Long bufferSize, FT_UInt pixelSize, uint8_t vertexIndex)
{
...
ftgxCharData *FreeTypeGX::cacheGlyphData(wchar_t charCode)
{
	...
	if (!FT_Load_Glyph(ftFace, gIndex, FT_LOAD_DEFAULT | FT_LOAD_RENDER | FT_LOAD_TARGET_LIGHT))
	{
		if(ftSlot->format == FT_GLYPH_FORMAT_BITMAP)
		{
			FT_Bitmap *glyphBitmap = &ftSlot->bitmap;

			uint16_t textureWidth = adjustTextureWidth(glyphBitmap->width);
			uint16_t textureHeight = adjustTextureHeight(glyphBitmap->rows);
...
uint16_t FreeTypeGX::drawText(int16_t x, int16_t y, wchar_t *text, GXColor color, uint16_t textStyle)
{
	...
	setVertexFormat(this->vertexIndex);
